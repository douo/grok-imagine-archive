...
- 日志写入 `archive/accounts/demo/logs/web.log`
...
如果错误里出现 `Cloudflare challenge detected`，说明当前请求被
Cloudflare 挑战页拦截。需要刷新浏览器会话，把新的 `cf_clearance` 写入本地账号配置，并确保
`user_agent`、`browser` 和代理设置与获取该 clearance cookie 的浏览器和出口 IP 一致。
